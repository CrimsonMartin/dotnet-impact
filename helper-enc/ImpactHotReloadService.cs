// Vendored from dotnet/roslyn @ 35d9211b (the commit behind the
// Microsoft.CodeAnalysis.Features 5.9.0 package), MIT licensed:
// src/Features/ExternalAccess/HotReload/Api/HotReloadService.cs
//
// This assembly deliberately takes the identity
// Microsoft.CodeAnalysis.ExternalAccess.HotReload (name + Roslyn public key,
// public-signed) because Features grants InternalsVisibleTo to exactly that
// identity — the supported access channel for Roslyn's Edit-and-Continue
// engine outside Visual Studio (dotnet-watch consumes it the same way).
// CoreCLR does not validate strong-name signatures, so public-signing with
// the published public key is sufficient. Only the class is renamed (public
// ImpactHotReloadService); the body is kept verbatim for diffability against
// upstream.
//
...
public sealed class ImpactHotReloadService(SolutionServices services, Func<ValueTask<ImmutableArray<string>>> capabilitiesProvider)
{
    ...
    public ImpactHotReloadService(HostWorkspaceServices services, ImmutableArray<string> capabilities)
    ...
    internal readonly struct TestAccessor(ImpactHotReloadService instance)
    {
